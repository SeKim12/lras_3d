...
def parse_args():
    ...
    parser.add_argument("--azimuth", type=float, default=30,
                        help="Azimuth angle in degrees.")
    parser.add_argument("--tilt", type=float, default=0,
                        help="Tilt angle in degrees.")
    parser.add_argument("--T_world", type=float, nargs='+', default=[0.1, 0.0, 0.0],
...
if __name__ == '__main__':
    ...
    for runs in range(args.num_runs):
        ...
        with torch.no_grad():
            ...
            rgb1_pred, flow_map, unmask_indices, indices_flow_in_256, segment_map, cum_log_prob = model_object_motion_cf.run_forward_with_RT(image0, pts_0, three_points_on_ground, R_world, T_world, K, condition_rgb=False, new_segment_sampling=True,
                    condition_from_nvs=False, full_segment_map=True)
            ...
            rgb1_pred.save(output_path + '/results/' + annotations.split('/')[-1].split('.')[0] + '_seed_' + str(runs) + '_nvs.png')
